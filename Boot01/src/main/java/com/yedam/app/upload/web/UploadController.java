...
public class UploadController {
	...
	public String formUploadFile(@RequestPart MultipartFile[] files) {
		for(MultipartFile file : files) {
			log.info(file.getContentType()); //log 메소드를 활용해서 정보출력 
			...
			Path savePath = Paths.get(saveName); // Path는 순수한 경로
			
			try {
				file.transferTo(savePath); // transferTo
			} catch(IOException e) {
				e.printStackTrace();
			}
		}
	...
	public List<String> uploadFile(@RequestPart MultipartFile[] uploadFiles) {
	    ...
	    for(MultipartFile uploadFile : uploadFiles){ //image파일이 아닐시에 return null
	    	if(uploadFile.getContentType().startsWith("image") == false){
	    		System.err.println("this file is not image type");
	    		return null;
	        }
	  
	        String fileName = uploadFile.getOriginalFilename();//getOriginalFilename 파일명만 넘겨옴
